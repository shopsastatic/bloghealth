...
export const NC_POST_FULL_FRAGMENT = gql(/* GraphQL */ `
	fragment NcmazFcPostFullFields on Post {
		...
		postData {
			...
			layoutStyle
			products {
				... on PostDataProductsItemLayout_Fields {
				title
				name
				image {
					node {
				...
				salePercentage
				actionButtons {
					... on PostDataProductsActionButtonsButtonLayout {
					actionText
					actionLink
					}
				}
				prosCons {
					... on PostDataProductsProsCons {
					pros
					cons
					}
				}
				description {
					...on PostDataProductsDescription {
						content
						shortDescription
					}
				}
